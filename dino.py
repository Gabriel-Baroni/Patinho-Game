...
pygame.display.set_caption("Joguinho do Patinho")

sprite_sheet =pygame.image.load(os.path.join(diretorio_imagens,"PATINHO GAME.png")).convert_alpha()
...
def reiniciar_jogo():
    ...
    velocidade_jogo=10
    colidiu=False
    dino.rect.y=ALTURA - 64 - 86.4//2
    dino.pulo=False
    dinovoador.rect.x=LARGURA
    cacto.rect.x=LARGURA
    cacto2.rect.x=LARGURA
    escolha_obstaculo=choice([0,1,2,3])
...
class Dino(pygame.sprite.Sprite):
    def __init__(self):
        ...
        for i in range(3):
            img = sprite_sheet.subsurface((i * 32,0), (32,28.8))
            img = pygame.transform.scale(img, (32*3, 28.8*3))
        ...
        self.pos_y_inicial = ALTURA - 64 - 86.4//2
...
class Nuvens(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = sprite_sheet.subsurface((10 * 32, 0), (32, 28.8))
        self.image = pygame.transform.scale(self.image, (32*3, 28.8*3))
        self.rect = self.image.get_rect()
...
class Chao(pygame.sprite.Sprite):
    def __init__(self, pos_x):
        pygame.sprite.Sprite.__init__(self)
        self.image = sprite_sheet.subsurface((9 * 32, 0), (32, 28.8))
        self.image = pygame.transform.scale(self.image, (32*2, 28.8*2))
        self.rect = self.image.get_rect()
...
class Cacto(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = sprite_sheet.subsurface((7 * 32, 0), (32, 28.8))
        self.image = pygame.transform.scale(self.image, (32*2, 28.8*2))
        self.rect = self.image.get_rect()
        self.mask=pygame.mask.from_surface(self.image)
        ...
        self.rect.x= LARGURA
    def update(self):
        if self.escolha==0 or self.escolha==2:
            if self.rect.topright[0] < 0:
                self.rect.x = LARGURA
            self.rect.x -= velocidade_jogo

class Cacto_2(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = sprite_sheet.subsurface((8 * 32, 0), (32, 28.8))
        self.image = pygame.transform.scale(self.image, (32*3, 28.8*3))
        self.rect = self.image.get_rect()
        self.mask=pygame.mask.from_surface(self.image)
        self.escolha=escolha_obstaculo
        self.rect.center=(LARGURA+500, ALTURA-64)
        self.rect.x= LARGURA
    def update(self):
        if self.escolha==3:
            if self.rect.topright[0] < 0:
...
cacto2=Cacto_2()
todas_as_sprites.add(cacto2)

dinovoador= Dino_voador()
todas_as_sprites.add(dinovoador)

grupo_obstaculos= pygame.sprite.Group()
grupo_obstaculos.add(cacto)
grupo_obstaculos.add(cacto2)
...
while True:
    ...
    if cacto.rect.topright[0]<=0 or cacto2.rect.topright[0] <=0 or dinovoador.rect.topright[0]<=0:
        escolha_obstaculo=choice([0,1,2,3])
        cacto.rect.x=LARGURA
        cacto2.rect.x=LARGURA+200
        dinovoador.rect.x=LARGURA
        cacto.escolha=escolha_obstaculo
        cacto2.escolha=escolha_obstaculo
